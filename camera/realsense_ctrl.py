...
class RealsenseCtrl():
    """RealSenseを使うときのclass

    RealSenseを使いやすくするために作成

    Attributes:
        connected_serial (list): 接続されているリアルセンスのシリアル一覧。 get_infoで取得
        connected_asic_serial_number (list): 接続されているリアルセンスのシリアル一覧。 get_infoで取得.
    ...
    def connect_rs(self, serial='', width=640, height=480):
        # init streaming
        config = (rs.config())
        if not serial == '':
